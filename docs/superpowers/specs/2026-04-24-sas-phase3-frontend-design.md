...
- **Flowing lines, anchored observations.** All time-series line charts use a gentle smooth curve (ECharts `smooth: 0.4` or equivalent) with visible data-point dots at every real observation. The curve matches the rounded visual language of the app (8px radii, soft shadows, Lumin accents). The visible dots preserve the honesty tenet (§5 main spec) — viewers can see exactly where real values are, with the curve between acting as a visual guide rather than a claim about interpolated values. Hover enlarges the dot (scale ~1.4) with a white border for tactile feedback. **Never use `step: "end"` / `step: "start"`** — hard-angle step lines conflict with the visual language; honesty is preserved via the visible dots instead. This applies to all 10 starter widgets and every widget that ships thereafter.
